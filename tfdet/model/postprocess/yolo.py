...
class FilterDetection(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        score_pred, logit_pred, bbox_pred, anchors = inputs
        if isinstance(score_pred, (list, tuple)):
            score_pred = tf.concat(score_pred, axis = -2)
        if isinstance(logit_pred, (list, tuple)):
            logit_pred = tf.concat(logit_pred, axis = -2)
        ...
        if self.valid_inside_anchor:
            ...
            score_pred = tf.gather(score_pred, valid_indices, axis = 1)
            logit_pred = tf.gather(logit_pred, valid_indices, axis = 1)
            bbox_pred = tf.gather(bbox_pred, valid_indices, axis = 1)
            anchors = tf.gather(anchors, valid_indices)
        y_pred = tf.multiply(logit_pred, score_pred)
        ...
        if not self.tensorrt:
            ...
            bbox_pred = yolo2bbox(anchors, bbox_pred, clip_ratio = self.clip_ratio)
